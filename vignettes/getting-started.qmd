...
Human-readable ICD-10-CM and ICD-9-CM diagnosis descriptions are bundled with the package. Run `tools/update_icd_lookups.R` from the repository root when refreshing official CDC/CMS lookup data.
...
This opens the timeline viewer in your browser. Start typing a patient ID (try `PAT` to see autocomplete suggestions with the sample data) and click "Load Patient". The autocomplete shows matching PATIDs with DOB and sex details after 2 characters.
...
| Diagnoses | Coral (#e74c3c) | ICD-9/10 diagnosis codes (with automatic ICD description lookup) |
| Procedures | Purple (#9b59b6) | CPT, HCPCS, ICD procedure codes |
| Labs | Green (#27ae60) | Lab results with values (abnormal results highlighted) |
...
- **Color By**: Choose between "Event Type" (default color scheme above) and "Source System" coloring (events colored by contributing data source)
- **Show Event Labels**: Toggle text labels on timeline markers; when off, point events appear as compact dots
- **Aggregation**: Individual, Daily, or Weekly event grouping
- **Auto-Clustering**: Automatically cluster nearby events as you zoom in/out

### Overview Minimap

Below the main timeline, a miniature overview shows the full date range with:

- **Draggable viewport**: Pan the main timeline by dragging the blue viewport rectangle
- **Filter range handles**: Orange handles at the edges that set the date range filter when dragged; changes sync bidirectionally with the date input fields

### Filtering Options

#### Event Type Checkboxes

Toggle visibility of each event type using checkboxes that display event counts.

#### Source System Filter

When a patient has data from multiple source systems, filter by contributing system. Events show colored left-border indicators identifying their source.
...
- **Resize**: Drag the bottom edge of the timeline pane to adjust height
